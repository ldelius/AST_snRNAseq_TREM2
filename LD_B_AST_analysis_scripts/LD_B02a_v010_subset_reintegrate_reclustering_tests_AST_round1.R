message("\n\n##########################################################################\n",
        "# Start B02a: Basic processing and subclustering from scFlow ", Sys.time(),
        "\n##########################################################################\n",
        "\n    \n",
        "\n##########################################################################\n\n")
...
### load  seurat
seur = qread(file = paste0(main_dir, "data_TREM2_michael/B_load_from_scflow_subcluster/B01_seur.qs")) # loads the Seurat object that was produced previously

###cell type for subsetting
celltype_subset <- "Astro"
...
###########################################################
#subset dataset, integrate data with Harmony
###########################################################

seur = seur[,seur$cluster_celltype == celltype_subset] # keeps only the astrocytes

message("\n\n          *** Normalising dataset... ", Sys.time(),"\n\n")

options(future.globals.maxSize = 1024^3) #SCTransform exceeds default memory limit for parallelisation with futures

seur[["RNA"]] <- split(seur[["RNA"]], f = seur$sample) # split by sample to account/normalise for sample structure

set.seed(123)

# normalise per sample (SCTransform)
...
# reduces the variable genes identified to a smaller number of principle components

set.seed(123)

# harmony corrects batch effects between samples. 
...
seur <- JoinLayers(seur, assay = "RNA") # merges the previousyl split RNA layers back into a single RNA assay
...
set.seed(123)
seur <- RunPCA(seur,  assay = "SCT", reduction.name = "pca") # runs PCA again on the re-normalized integrated data
