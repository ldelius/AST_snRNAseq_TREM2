message("\n\n##########################################################################\n",
        "# Start B02: Basic processing and subclustering from scFlow ", Sys.time(),
        "\n##########################################################################\n",
        "\n    \n",
        "\n##########################################################################\n\n")
...
### load  seurat
seur = qread(file = paste0(out_dir, "LD_B02a_seur.qs")) # change

###subset dataset (remove clusters expressing other cell type markers)

seur = seur[,!(seur$SCT_snn_res.1.5 %in% c(14, 15, 17, 21, 26, 30, 31, 33, 38, 40))]
