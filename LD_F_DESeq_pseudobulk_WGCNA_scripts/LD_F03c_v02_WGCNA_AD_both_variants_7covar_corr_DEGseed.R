...
out_dir = paste0(main_dir,"LD_F_DESeq_pseudobulk_WGCNA/LD_F03c_v02/")
...
### (same per-sample resolution as above, but columns split into
###  CV | R47H | R62H blocks instead of reading by cluster)
#######################################################

# split vector aligned to eigengene matrix columns
...
######################################################################
### Module activity by group — pooled across clusters
### Dots = all pseudobulk cluster_samples (pseudoreplicated for
### visualisation). Statistical annotation = LMM omnibus p-value from
### the donor-aware LMM above, which handles pseudoreplication via
### random effects (1|cluster_name) + (1|sample).
######################################################################

message("\n\n   *Generate module activity by group plot \n")

me_mat_act = bulk_data$wgcna$mod_eigengene_mat
meta_act   = bulk_data$meta

# long format: one row per module × pseudobulk
t_act = do.call(rbind, lapply(rownames(me_mat_act), function(mod1){
  data.frame(
    module       = mod1,
    eigengene    = as.numeric(me_mat_act[mod1, meta_act$cluster_sample]),
    TREM2Variant = meta_act$TREM2Variant,
    stringsAsFactors = FALSE
  )
}))

# sort modules numerically (M1, M2, ... M10, M11) not alphabetically
mods_ordered = mods[order(as.numeric(gsub("M", "", mods)))]
t_act$module = factor(t_act$module, levels = mods_ordered)

# LMM omnibus p-value label per module (computed in the LMM section above)
if (!is.null(lmm_omni) && nrow(lmm_omni) > 0){
...
gr_levels = intersect(c("CV", "R47H", "R62H"), unique(t_act$TREM2Variant))

act_plot = function(dat, mod_levels, title){
  dat$module = factor(dat$module, levels = mod_levels)
  ggplot(dat, aes(x = TREM2Variant, y = eigengene)) +
    geom_boxplot(aes(fill = TREM2Variant), outlier.shape = NA, alpha = 0.6,
                 color = "grey30", width = 0.6) +
    geom_point(aes(fill = TREM2Variant), shape = 21,
               position = position_jitter(width = 0.15, height = 0),
               size = 0.8, stroke = 0.2, color = "grey30") +
    geom_hline(yintercept = 0, linetype = "dashed", color = "grey60", linewidth = 0.3) +
    geom_text(data = distinct(dat, module, annot),
              aes(x = Inf, y = Inf, label = annot),
              inherit.aes = FALSE,
              hjust = 1.1, vjust = 1.5, size = 2.5, color = "grey20") +
    scale_x_discrete(limits = gr_levels) +
    scale_fill_manual(limits = gr_levels, values = pal(gr_levels)) +
    ...
    labs(title = title, x = "group", y = "activity")
}

# combined: all modules
pdf(file   = paste0(out_dir, script_ind, "Module_activity_by_group.pdf"),
    width  = 4,
    height = max(4, length(mods_ordered) * 1.2 + 1))
plot(act_plot(t_act, mods_ordered, "Module activity by group"))
dev.off()

# subset: selected modules of interest
mods_subset  = intersect(mods_ordered,
                         c("M1","M5","M8","M11","M12","M14","M15"))

pdf(file   = paste0(out_dir, script_ind, "Module_activity_by_group_subset.pdf"),
    width  = 4,
    height = max(4, length(mods_subset) * 1.2 + 1))
plot(act_plot(t_act[t_act$module %in% mods_subset, ], mods_subset,
              "Module activity by group (selected)"))
dev.off()



#########################################
# (omitted in F03c) all-genes-by-module gene Z-score heatmap
#   - impractical at ~26k genes (rows); per-user request. The col_annot used by the
#     downstream GO-gene / GOI / subtype heatmaps is recreated in the GO-genes section below.
#########################################



#################################################
### GO-BP over-representation analysis of module genes (open GO-BP)
#################################################

GO_list = list()
GO_results_tab = NULL

# v02 keeps the grey/unassigned module M0 in the GO analysis (matches F03a1; per user request).
...
pdf(file = paste0(out_dir,script_ind, "GO_results_by_module_network_plot.pdf"),
    width = 15, height = 12)
# render per-plot with tryCatch: some emapplots fail at render (ggrepel "zero dimension
# viewport") for degenerate term-networks; skip those rather than halt the whole script.
for (nm in names(pl)){
  tryCatch(print(pl[[nm]]),
           error = function(e) message("    skipping network-plot render for ", nm, ": ", conditionMessage(e)))
}
dev.off()
