...
# Donor-aware TREM2 LMM results from the compute script.
...
### 1) Module-trait correlation heatmap
...
### BH-FDR across the two displayed variant-versus-CV contrasts
...
for (mod1 in mod_names){
  # Pairwise contrasts distinguish the two variants; the omnibus test does not.
...
# Display labels only; matrix names are retained for indexing.
...
# Custom colour and significance legend.
...
draw_cor_plot = function(newpage = TRUE){
  draw(ht_cor, annotation_legend_list = list(combined_legend_cor), annotation_legend_side = "right",
      newpage = newpage)
  # Highlight selected module rows across the full heatmap width.
...
# Allow space for the rotated leftmost column label.
...
### 2) TREM2Variant module eigengene heatmap
...
### 2b) Combined eigengene and module-trait heatmaps
