message("\n\n##########################################################################\n",
        "# Start G02b: CellChat by TREM2Variant ", Sys.time(),
        "\n##########################################################################\n",
        "\n   LD_G02a - ungrouped\n",
        "     LD_G02b - TREM2Variant (CV / R47H / R62H) (this script)\n",
...
###########################################################
# 1a. load merged Seurat, set samples column
###########################################################
...
###########################################################
# 2. cell-count diagnostics per TREM2Variant split
###########################################################

#Important here: small variant subgroups may have <10 cells in some clusters
#after the cap, which filterCommunication will then drop. Save the table to
#cross-reference with G02b output.
 
seur@meta.data %>% as_tibble() %>%
  count(cluster_name, TREM2Variant, name = "n_cells") %>%
  pivot_wider(names_from = TREM2Variant, values_from = n_cells, values_fill = 0) %>%
  write_csv(paste0(out_dir, script_ind, "cells_per_cluster_per_TREM2Variant.csv"))
...
run_cellchat = function(seur_in, run_label, db_use,
  ...
  #seed.use is CellChat's parallel-safe seed
...
###########################################################
# 5. RUN: split by TREM2Variant, loop over splits
###########################################################

message("\n\n##########################################################################\n",
        "# RUN: CellChat by TREM2Variant ", Sys.time(),
        "\n##########################################################################\n\n")


stopifnot("TREM2Variant" %in% colnames(seur@meta.data))

seur$group_TREM2 = as.character(seur$TREM2Variant)
seur_list_TREM2 = SplitObject(seur, split.by = "group_TREM2")

#drop original Seurat now that splits are made - SplitObject duplicates cells
rm(seur); gc()

cat("\nTREM2Variant splits:\n")
for (g in names(seur_list_TREM2)){
  cat("  ", g, ":", ncol(seur_list_TREM2[[g]]), "cells\n")
}
...
for (g in names(seur_list_TREM2)){
  ...
  cc = run_cellchat(
    seur_in      = seur_list_TREM2[[g]],
    run_label    = paste0("TREM2Variant_", g),
